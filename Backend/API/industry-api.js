...
industryApp.use(exp.json());

let createquestionscollection;
let usercollection;

industryApp.use((req, res, next) => {
...
industryApp.post('/submit-form', verifyToken, expressAsyncHandler(async (req, res) => {
  ...
  try {
    const form = await createquestionscollection.findOne({ title: 'Industry form' });
...
industryApp.get('/get-form',verifyToken, expressAsyncHandler(async (req, res) => {
  try {
      const form = await createquestionscollection.findOne({title:"Industry form"});
      console.log(form)
      if (!form) {
...
industryApp.get('/get-form-response-stats', verifyToken, expressAsyncHandler(async (req, res) => {
  try {
