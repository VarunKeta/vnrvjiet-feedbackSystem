...
studentApp.post('/submit-theory-form', verifyToken, expressAsyncHandler(async (req, res) => {
  const {username, responses, comments} = req.body;
  ...
  try {
      const form = await createquestionscollection.findOne({ title: 'Student form (Theory)' });
...
studentApp.post('/submit-laboratory-form', verifyToken, expressAsyncHandler(async (req, res) => {
  const { username, responses, comments} = req.body;
  const submission = {
    username,
    responses,
  ...
  try {
      const form = await createquestionscollection.findOne({ title: 'Student form (Laboratory)' });
...
studentApp.get('/get-theory-form',verifyToken, expressAsyncHandler(async (req, res) => {
  try {
      const form = await createquestionscollection.findOne({title:"Student form (Theory)"});
      console.log(form)
      if (!form) {
...
studentApp.get('/get-laboratory-form',verifyToken, expressAsyncHandler(async (req, res) => {
  try {
      const form = await createquestionscollection.findOne({title:"Student form (Laboratory)"});
      console.log(form)
      if (!form) {
...
studentApp.get('/get-form-response-stats-theory', verifyToken, expressAsyncHandler(async (req, res) => {
  try {
...
studentApp.get('/get-form-response-stats-laboratory', verifyToken, expressAsyncHandler(async (req, res) => {
  try {
    // Retrieve the form data
    const form = await createquestionscollection.findOne({ title: "Student form (Laboratory)" });
