...
function Department() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axiosWithToken.get('http://localhost:5000/graduate-api/get-department-form');
        const { data } = response;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosWithToken.post('http://localhost:5000/graduate-api/submit-department-form', formData);
  ...
  return (
    <form onSubmit={handleSubmit} className="container w-75 mt-5 mb-5 cls1">
      <h1 className="text-center mb-1">Graduate Exit Feedback Form</h1>
      <h2 className="text-center mb-5">Department</h2>
      <p>Department and its facilities are the resources where the student has a close experience during the active teaching and learning methodology. These are helpful in the execution of the curriculum and evaluation of Program Educational Objectives (PEO’s) and Program Outcomes (PO’s) </p>
      <div className="card p-4 mb-4">
        ...
        <div className="mb-3">
          ...
          <input
            ...
            className="form-control"
            name="registrationno"
            value={formDetails.registrationno}
            onChange={(e) => setFormDetails({ ...formDetails, registrationno: e.target.value })}
          />
        </div>
        <div className="mb-3">
