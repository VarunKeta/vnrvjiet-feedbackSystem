...
function Institution() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axiosWithToken.get('http://localhost:5000/graduate-api/get-institution-form');
        const { data } = response;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosWithToken.post('http://localhost:5000/graduate-api/submit-institution-form', formData);
  ...
  return (
    <form onSubmit={handleSubmit} className="container w-75 mt-5 mb-5 cls1">
      <h1 className="text-center mb-2">Graduate Exit Feedback Form</h1>
      <h2 className="text-center mb-4">Institution</h2>
      <p>As an institute, it is our cherished mission to see that student should be provided needed facilities and assistance required</p>
