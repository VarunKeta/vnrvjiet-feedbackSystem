...
function Industry() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axiosWithToken.get('http://localhost:5000/industry-api/get-form');
        const { data } = response;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosWithToken.post('http://localhost:5000/industry-api/submit-form', formData);
  ...
  return (
    <form onSubmit={handleSubmit} className="container w-75 mt-5 mb-5 cls1">
      <h1 className="text-center mb-5">Industry Personnel/Employer Feedback Form</h1>
