...
function Theory() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axiosWithToken.get('http://localhost:5000/student-api/get-theory-form');
        const { data } = response;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosWithToken.post('http://localhost:5000/student-api/submit-theory-form', formData);
  ...
  return (
    <form onSubmit={handleSubmit} className="container w-75 mt-5 mb-5 cls1">
      <h1 className="text-center mb-1">Student Feedback Form</h1>
      <h2 className="text-center mb-5">Theory</h2>
