...
function Lab() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axiosWithToken.get('http://localhost:5000/student-api/get-laboratory-form');
        const { data } = response;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axiosWithToken.post('http://localhost:5000/student-api/submit-laboratory-form', formData);
